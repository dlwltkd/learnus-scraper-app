...
- **A web ticket belongs to its initiating browser.** Completion also requires the helper-set `__Host-luconnect_login` HttpOnly cookie to match the ticket (`luconnect_login` when `WEB_SESSION_COOKIE_SECURE=false` for localhost). Origin checks alone do not prevent a copied login link from signing another browser into the sender's account. Consume the ticket atomically and clear the temporary cookie on success. Helper 0.1.1 or later is required; do not restore an unbound fallback.
