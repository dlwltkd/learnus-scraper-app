...
- All text in Korean (해요체), under 50 chars each"""
    ...
    def caption_slide(self, image_path: str, lecture_title: str = "", page_no: int = 0) -> tuple[str, dict]:
        ...
        prompt = (
            ...
            "actually shown. Transcribe any readable text in the figure.\n\n"
            "Critical: if any text is struck through, crossed out, marked with an X, or "
            "otherwise presented as wrong, say so explicitly and state that the lecture is "
            "rejecting it. Extracted text loses this, so a struck-out sentence reaches the "
            "reader as an assertion and means the exact opposite of what was taught. The "
            "same goes for anything labelled 반례, 틀림, wrong, don't, or ✗.\n\n"
            "Otherwise ignore decoration — colours, fonts and layout that carry no meaning — "
            "and do not begin with phrases like 'This slide shows'.\n\n"
